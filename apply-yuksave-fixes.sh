#!/usr/bin/env bash
# Terapkan perbaikan pre-production YukSave. Jalankan dari root folder
# project (folder yang ada package.json-nya). Aman dijalankan berkali-kali.
set -e
...
mkdir -p lib/parsers app/api/download

# 1) Hapus proxy.ts yang salah nama/export (Next.js tidak pernah menjalankannya)
rm -f proxy.ts

# 2) middleware.ts — nama file & export yang benar supaya CSP benar-benar aktif
cat > middleware.ts << 'YUKSAVE_PATCH_EOF'
...
// NOTE: this file must be named `middleware.ts` and export a function
// named `middleware` — that's the only file name/export combo Next.js
// actually recognizes and runs automatically. It was previously named
// `proxy.ts` exporting `proxy`, which Next.js silently ignored: the CSP
// header below was never being sent in production. Keep the name/export
// exactly as-is unless you've confirmed the header shows up in a real
// response (e.g. `curl -I` your deployed URL and check for
...
# 3) lib/parsers/tiktok.ts — retry tikwm + scrapecreators masuk parserChain
...
# 4) app/api/download/route.ts — catatan allowlist untuk fallback baru
cat > app/api/download/route.ts << 'YUKSAVE_PATCH_EOF'
...
# 5) .env.local.example — dokumentasikan RAPIDAPI_KEY & SCRAPECREATORS_API_KEY
...
# Optional fallback parsers for lib/parsers/tiktok.ts, used only when
# tikwm (the free, keyless parser) is down or blocks a request. Leave
# blank to skip that fallback entirely — parserChain just throws past it
# immediately, no code change needed. WITHOUT AT LEAST ONE OF THESE SET,
# the app has no real fallback if tikwm goes down or rate-limits you —
# see the README's "Parser TikTok tidak resmi" note before going live.
# - RAPIDAPI_KEY: replace the placeholder host/endpoint in
#   parseWithRapidApi (lib/parsers/tiktok.ts) with whichever TikTok
#   scraper you actually subscribe to on RapidAPI first.
...
# 6) README.md — update catatan parser chain
...
# 7) Hapus script fix lama yang sudah usang/setengah diterapkan
rm -f terapkan-perbaikan.sh

echo "Selesai. Semua perbaikan sudah diterapkan."
echo "Jangan lupa isi RAPIDAPI_KEY / SCRAPECREATORS_API_KEY di .env.local sebelum production."
